...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/MBP/Box Sync/PHSU/Design and stats consultation/Eliut Rivera/Suicide and Emotional Regulation Project/Free recall task/free recall task.py',
...
practice_word_list = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('practice_trials.csv'),
    seed=None, name='practice_word_list')
thisExp.addLoop(practice_word_list)  # add the loop to the experiment
thisPractice_word_list = practice_word_list.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisPractice_word_list.rgb)
if thisPractice_word_list != None:
    for paramName in thisPractice_word_list:
        exec('{} = thisPractice_word_list[paramName]'.format(paramName))
print(thisPractice_word_list)

for thisPractice_word_list in practice_word_list:
    currentLoop = practice_word_list
    # abbreviate parameter names if possible (e.g. rgb = thisPractice_word_list.rgb)
    if thisPractice_word_list != None:
        for paramName in thisPractice_word_list:
            exec('{} = thisPractice_word_list[paramName]'.format(paramName))
    ...
    for thisComponent in ISIComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
# completed 1 repeats of 'practice_word_list'
...
thisExp.addData('key_resp_exp.keys',key_resp_exp.keys)
if key_resp_exp.keys != None:  # we had a response
    thisExp.addData('key_resp_exp.rt', key_resp_exp.rt)
thisExp.nextEntry()
resp_feedback_text = ''
...
# set up handler to look after randomisation of conditions etc
exp_block_loops = data.TrialHandler(nReps=len(block_ranges_exp), method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='exp_block_loops')
thisExp.addLoop(exp_block_loops)  # add the loop to the experiment
thisExp_block_loop = exp_block_loops.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisExp_block_loop.rgb)
if thisExp_block_loop != None:
    for paramName in thisExp_block_loop:
        exec('{} = thisExp_block_loop[paramName]'.format(paramName))

for thisExp_block_loop in exp_block_loops:
    currentLoop = exp_block_loops
    # abbreviate parameter names if possible (e.g. rgb = thisExp_block_loop.rgb)
    if thisExp_block_loop != None:
        for paramName in thisExp_block_loop:
            exec('{} = thisExp_block_loop[paramName]'.format(paramName))
    ...
    if key_resp_exp.keys in ['', [], None]:  # No response was made
        key_resp_exp.keys=None
    exp_block_loops.addData('key_resp_exp.keys',key_resp_exp.keys)
    if key_resp_exp.keys != None:  # we had a response
        exp_block_loops.addData('key_resp_exp.rt', key_resp_exp.rt)
    resp_feedback_text = ''
    current_word_list.clear()
    current_word_list_valence.clear()
    # the Routine "response_participant" was not non-slip safe, so reset the non-slip timer
...
# completed len(block_ranges_exp) repeats of 'exp_block_loops'
